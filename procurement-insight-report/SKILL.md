...
1. `weekly_quote_report` for weekly summary counts and headline metrics
2. `quote_details` for quote-level drill-down
3. `search_quotes` for scoped search and supporting retrieval
4. `pending_quotes` for backlog and operational context
5. `search_procurement` for assignee lookup
